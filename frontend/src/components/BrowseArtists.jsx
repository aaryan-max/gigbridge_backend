...
export default function BrowseArtists() {
    ...
    const fetchArtists = async () => {
      ...
      try {
        ...
        if (data && data.freelancers) {
          setArtists(data.freelancers.map(f => ({
            id: f.id || f.email,
            name: f.name || 'Unknown',
            role: f.tagline || f.category || 'Freelancer',
            category: f.category || 'Other',
            priceRange: f.hourly_rate ? `₹${f.hourly_rate}/hr` : 'Contact',
            availability: f.availability || 'Unknown',
            location: f.location || 'India',
            bio: f.bio || '',
            tags: (f.skills || '').split(',').filter(Boolean),
            rating: f.rating || 4.5,
            avatar: f.profile_picture || 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=256&auto=format&fit=crop',
            online: true,
          })));
        }
      } catch (error) {
  ...
  const filtered = useMemo(() => {
    ...
    return byAvail;
  }, [artists, q, sort, selCats, budget, selLocs, avail]);
